document.addEventListener("DOMContentLoaded", ()=>{
	DAN.show('item_image', 'tigers_container')
    DAN.show('item_image', 'princess_container')
    DAN.show('item_image', 'leopards_container')
    PROCESS.init()
});
...
const PROCESS = {
	...
	images_loading() {
		...
		DAN.ajax('/system/object_detection_upload_ajax', form, (data) => {
	...
	images_processing() {
		...
		DAN.ajax('/system/object_detection_processing_ajax', form, (data) => {
			DAN.modal.add('ОБРАБОТКА ЗАВЕРШЕНА!')
			document.location.href = '/system/object_detection'
		})
	}
}